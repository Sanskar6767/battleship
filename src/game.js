...
export class Game {
    ...
    playTurn(x, y) {
        ...
        this.currentPlayer.attack(opponentBoard, x, y, 'human');
        this.switchPlayer();
    ...
    checkWin(player) {
        return player.gameboard.allSunk();  // Ensure this checks the total number of ships
    }
    ...
    computerMove() {
        let x, y;
        do {
            x = Math.floor(Math.random() * this.player1.gameboard.size);
            y = Math.floor(Math.random() * this.player1.gameboard.size);
        } while (this.player1.gameboard.isAlreadyAttacked(x, y));
    
        this.player2.attack(this.player1.gameboard, x, y, 'computer');
    }
